...
namespace LLLoader.PortableExecutable
{
    [Serializable]
    public class PortableExecutable :  MemoryIterator
    {
        public IMAGE_NT_HEADER32 NTHeader { get; }
            ...
            : base(data)
        {
            ...
            if (Read(out tempDosHd) && tempDosHd.e_magic == Constants.DOS_SIGNATURE) //first check, DOS header ('MZ')
            {
                ...
                {
                    if (tempNtHd.OptionalHeader.Magic == Constants.PE32_FORMAT || tempNtHd.OptionalHeader.Magic == Constants.PE32P_FORMAT) //make sure it's a PE or PE+ file
                    {
        ...
        public IEnumerable<IMAGE_SECTION_HEADER> EnumSectionHeaders()
        {
            ...
            {
                if (Read(pSections + (i * szSection), SeekOrigin.Begin, out pSecHd))
                    yield return pSecHd;
            }
        }
        ...
        {
            IMAGE_DATA_DIRECTORY impDir = NTHeader.OptionalHeader.DataDirectory[(int)DATA_DIRECTORIES.ImportTable];
            ...
            while (Read(pDesc, SeekOrigin.Begin, out IMAGE_IMPORT_DESCRIPTOR desc) && desc.OriginalFirstThunk > 0 && desc.Name > 0)
            {
                yield return desc;
                pDesc += szDesc;
            }
